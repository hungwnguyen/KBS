...
// Hàm chọn (Selection)
function tournamentSelection(population_featureFitness) {
  const selectedpopulation_feature = [];
  const tournamentSize = 8; // Kích thước của mỗi giải đấu
  const randomIndexDic = {};
  while (selectedpopulation_feature.length < 2) {
    // Bước 2.1: Randomly choose individuals for the tournament
    const tournamentParticipants = [];
    for (let i = 0; i < tournamentSize; i++) {
      let randomIndex = Math.floor(Math.random() * population_feature.length);
      while(Object.hasOwnProperty.call(randomIndexDic, randomIndex) && Object.keys(randomIndexDic).length !== population_feature.length){
        randomIndex = Math.floor(Math.random() * population_feature.length);
      }
      randomIndexDic[randomIndex] = true;
      tournamentParticipants.push({
        index: randomIndex,
        fitness: population_featureFitness[randomIndex],
      });
    }
    ...
    selectedpopulation_feature.push(population_feature[winner.index]);

    // Loại bỏ cá thể có độ thích nghi thấp nhất khỏi population_feature
    const minFitnessIndex = population_featureFitness.indexOf(Math.min(...population_featureFitness));
    population_feature.splice(minFitnessIndex, 1);
    population_featureFitness.splice(minFitnessIndex, 1);
  }

  return selectedpopulation_feature;
}
...
function mutation(offspring) {
  for (let i = 0; i < offspring.length; i++) {
    ...
    population_feature.push(offspring[i]);
  }
}
...
function sga_featureSimple(words){
  ...
  let fitnessScores = population_feature.map(individual => calculateFitness(individual));
  while (Math.max(...fitnessScores) != 100) {
    ...
    const selectedpopulation_feature = tournamentSelection(fitnessScores);
    ...
    fitnessScores = population_feature.map(individual => calculateFitness(individual));
  }
